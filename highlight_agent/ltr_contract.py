...
def validate_feature_contract(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    if metadata.get("schema_version") != LTR_FEATURE_SCHEMA_VERSION:
        mismatches.insert(
            0, f"schema_version={metadata.get('schema_version')!r} (expected {LTR_FEATURE_SCHEMA_VERSION!r})"
        )
    if mismatches:
    ...
    if not isinstance(l_ref, (int, float)) or float(l_ref) <= 0:
        raise LTRPipelineError("LTR_CHECKPOINT_SCHEMA_MISMATCH", "checkpoint metadata L_ref must be a positive number")
    if not metadata.get("dataset_fingerprint"):
        raise LTRPipelineError("LTR_CHECKPOINT_SCHEMA_MISMATCH", "checkpoint metadata is missing dataset_fingerprint")
    return expected

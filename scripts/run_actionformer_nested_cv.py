...
def run_outer_fold(
    *,
    ...
    report: dict[str, Any] = {
        "status": "running", "outer_fold": fold, "outer_split": splits,
        "started_at_unix": time.time(), "stage": "inner_localization",
        "config": {"localization": config.to_dict(), "scorer": scorer_config.to_dict(),
                   "seed": seed, "inner_folds": inner_fold_count, "loc_epochs": loc_epochs,
                   "ltr_epochs": ltr_epochs, "patience": patience, "device": device},
    ...
    try:
        ...
        for index, (fit, selection, targets) in enumerate(inner_splits(train, folds=inner_fold_count, seed=seed)):
            ...
            _, inner_report = train_actionformer_localization(
                train_examples=fit, val_examples=selection, config=config, **paths,
                max_epochs=loc_epochs, patience=patience, seed=seed, device=device,
            ...
            report["inner_runs"].append({
                "inner_fold": index, "best_epoch": inner_report["best_epoch"],
                "train": sorted(x.video_id for x in fit), "selection": sorted(x.video_id for x in selection),
        ...
        model, _ = train_actionformer_localization(
            train_examples=train, val_examples=[], config=config, **paths,
            max_epochs=fixed_budget, patience=patience, seed=seed, device=device,
            run_name=f"nested_fold{fold}_outer_refit", fixed_epochs=True, ancestor_lineage=ancestors,
        )
        ...
        cache = build_nested_proposal_cache(
            outer_fold=fold, train_examples=train, val_video_ids=splits["val"], test_video_ids=splits["test"],
            generators=generators, outer_checkpoint=outer_checkpoint,
            output_path=directory / "nested_proposals.json", device=device,
        )
        ...
        _, ltr_report = train_proposal_ltr(
            actionformer=model, checkpoint_metadata=metadata, train_examples=train, val_examples=val,
            **paths, max_epochs=ltr_epochs, learning_rate=2e-4, patience=patience,
            scorer_config=scorer_config, seed=seed, device=device, run_name=f"nested_fold{fold}_imsab",
            nested_cache_path=directory / "nested_proposals.json", source_checkpoint_path=outer_checkpoint,
...
def summarize(directory: Path, reports: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    _atomic_json(directory / "cv_summary.json", {"folds": rows, "summary": summary, "aggregation": "macro fold population SD"})
    write_training_history_csv(directory / "cv_metrics.csv", rows)
    lines = ["# Nested OOF v2 training report", "", "Protocol: separate inner fit/selection/prediction partitions per outer fold; validation predicted-only.",
             ...
             "|---:|---:|---:|---:|---:|"]
    for row in rows:
        lines.append(f'| {row["fold"]} | {row["val_predicted_ndcg3"]:.4f} | {row["confidence_map_iou_0_3"]:.4f} | {row["imsab_map_iou_0_3"]:.4f} | {row["imsab_recall_at_3_iou_0_3"]:.4f} |')
    lines.extend(["", "Every inner localization, outer refit and IMSAB run includes train_log.json, history.csv, curves.svg, best.pt and last.pt. The fixed-budget outer refit has no validation score by design.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", required=True, help="Must be a new directory; previous runs are never overwritten.")
    ...
    directory.mkdir(parents=True, exist_ok=False)
    _atomic_json(directory / "environment.json", {"python": platform.python_version(), "torch": torch.__version__,
                                                "platform": platform.platform(), "arguments": vars(args)})
    reports = []
    for fold in args.folds:
        manifest = args.manifest_template.format(fold=fold)
        data = {split: load_actionformer_manifest(manifest, split=split, project_root=args.project_root, ready_only=False)
                for split in ("train", "val", "test")}
        reports.append(run_outer_fold(
            **data, directory=directory / f"fold{fold}", fold=fold,
            config=ActionFormerConfig(d_model=32, attention_window=64), scorer_config=ProposalLTRConfig(),
            inner_fold_count=args.inner_folds, loc_epochs=args.loc_epochs, ltr_epochs=args.ltr_epochs,
